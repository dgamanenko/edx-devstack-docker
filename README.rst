...
1. The Docker Compose file mounts a host volume for each service's executing
   ...
   ``~/workspace/credentials``, etc. These repos can be cloned with the command
   below.
...
Course Discovery Service, you would modify ``DISCOVERY_VERSION`` (``COMMON_GIT_PATH`` for use another remote) in
...
.. |Build Status| image:: https://travis-ci.org/dgamanenko/edx-devstack-docker.svg?branch=master
   :target: https://travis-ci.org/dgamanenko/edx-devstack-docker
